...
class renderCSS {
	function render() {
		...
		NGSession::getInstance()->currentPath='classes/plugins/ngpluginnavigation/ngpluginnavigationvertical/css/';
		NGDBConnector::getInstance ()->connect ();
		
		$css = new NGRenderCSS ();
		$css->cacheId = 'navvertical'.$id;
				
		if (!$css->fetchFromCache ()) {
			include_once NGConfig::pluginsPath () . '/ngpluginnavigation/ngpluginnavigationvertical/ngpluginnavigationvertical.php';
			...
			$css->templateFilename = 'ngpluginnavigation/ngpluginnavigationvertical/tpl/css.tpl';
			$css->template->assign ( 'settings', $settings );
			$css->template->assign ( 'config', NGPluginNavigationVertical::parseConfig($settings->$property));
