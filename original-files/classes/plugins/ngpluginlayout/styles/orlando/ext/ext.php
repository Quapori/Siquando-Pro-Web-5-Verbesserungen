...
class NGPluginLayoutProExt {
	...
	public function render() {
		...
		$this->layout->javaScripts ['orlando'] = NGUtil::prependRootPath ( 'classes/plugins/ngpluginlayout/styles/orlando/js/orlando.js' );
		
		if (array_key_exists ( 'options.smoothwheel', $this->config )) {
			if (NGUtil::StringXMLToBool ( $this->config ['options.smoothwheel'] )) {
				$this->layout->javaScripts ['orlandowheel'] = NGUtil::prependRootPath ( 'classes/plugins/ngpluginlayout/styles/orlando/js/orlandowheel.js' );
			}
		}
	}
}