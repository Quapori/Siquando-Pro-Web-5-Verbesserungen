...
class NGPluginLayoutProExt
{
    ...
    public function render()
    {
        ...
        $this->layout->javaScripts ['novascotia'] = NGUtil::prependRootPath('classes/plugins/ngpluginlayout/styles/novascotia/js/novascotia.js');
        ...
        if (!$this->layout->topic->hideeyecatcher) {
            ...
            foreach ($bouquet->items as $item) {
                ...
                $pictures [] = NGLink::getPictureURL($item->displayPicture()->objectUID, 1200, 675, NGPicture::Ratio16by9);
            }

            if (count($pictures) === 0 && $this->layout->settings->config ['options.galeryinheritance'] == 'true') {
                $this->layout->createImage('eyecatcher', 1200, 675, NGPicture::Ratio16by9);
            }
