...
class NGPluginLayoutProExt {
	...
	public function render() {
		...
		$this->layout->javaScripts ['zagreb'] = NGUtil::prependRootPath ( 'classes/plugins/ngpluginlayout/styles/zagreb/js/zagreb.js' );
		...
		$this->layout->template->assign ( 'down', NGUtil::StringXMLToBool ( $this->layout->settings->config ['options.down'] ) );
		$this->layout->template->assign ( 'wait', $this->layout->settings->config ['options.wait'] );
		$this->layout->template->assign ( 'speed', $this->layout->settings->config ['options.speed'] );
		$this->layout->template->assign ( 'parallax', $this->layout->settings->config ['options.parallax'] );
		$this->layout->template->assign ( 'mutevideo', NGUtil::StringXMLToBool ( $this->layout->settings->config ['options.mutevideo'] ) );
		$this->layout->template->assign ( 'small', $this->layout->settings->config ['options.small'] );
		...
		foreach ( $bouquet->items as $item ) {
			...
			$pictures [] = NGLink::getPictureURL ( $item->displayPicture ()->objectUID, 1920, 1080, NGPicture::Ratio16by9 );
		}
		
		if (count ( $pictures ) === 0 && $this->layout->settings->config ['options.galeryinheritance'] == 'true') {
			$this->layout->createImage ( 'eyecatcher', 1920, 1080, NGPicture::Ratio16by9 );
		}
