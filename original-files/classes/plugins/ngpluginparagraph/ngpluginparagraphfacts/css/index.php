...
class renderCSS {
	function render() {
		...
		NGSession::getInstance()->currentPath='classes/plugins/ngpluginparagraph/ngpluginparagraphfacts/css/';
		NGDBConnector::getInstance ()->connect ();
		
		$css = new NGRenderCSS ();
		$css->cacheId = 'ngpluginfacts';
		...
		if (!$css->fetchFromCache ()) {
			...
			$css->templateFilename = 'ngpluginparagraph/ngpluginparagraphfacts/tpl/style.tpl';
