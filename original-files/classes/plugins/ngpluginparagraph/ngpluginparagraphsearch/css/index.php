...
class renderCSS {
	function render() {
		...
		NGSession::getInstance()->currentPath='classes/plugins/ngpluginparagraph/ngpluginparagraphsearch/css/';
		NGDBConnector::getInstance ()->connect ();

		$css = new NGRenderCSS ();
		$css->cacheId = 'ngpluginsearch';
		...
		if (!$css->fetchFromCache ()) {
			...
			$css->templateFilename = 'ngpluginparagraph/ngpluginparagraphsearch/tpl/style.tpl';
