...
class renderCSS {
	function render() {
		...
		NGSession::getInstance()->currentPath='classes/plugins/ngpluginparagraph/ngpluginparagraphaudiocharts/css/';
		NGDBConnector::getInstance ()->connect ();
		
		$css = new NGRenderCSS ();
		$css->cacheId = 'ngpluginaudiocharts';
		...
		if (!$css->fetchFromCache ()) {
			...
			$css->templateFilename = 'ngpluginparagraph/ngpluginparagraphaudiocharts/tpl/style.tpl';
