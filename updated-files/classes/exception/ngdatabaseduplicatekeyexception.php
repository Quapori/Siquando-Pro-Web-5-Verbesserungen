...
class NGDatabaseDuplicateKeyException extends NGException
{
    public $errorCode;
    public $errorMessage;
    public $sql;
    ...
    {
        if ($sql!=='') {
            parent::__construct(sprintf('Database access failed for duplicate key. Database error #%d: "%s". SQL: %s', $errorCode, $errorMessage, $sql), 30005);
        }
        else
        {
            parent::__construct(sprintf('Database access failed for duplicate key. Database error #%d: "%s".', $errorCode, $errorMessage), 30005);
        }
