...
class renderCSS {
	function render() {
		...
		NGSession::getInstance()->currentPath='classes/plugins/ngpluginnavigation/ngpluginnavigationsuperdropdown/css/';
		NGDBConnector::getInstance ()->connect ();
		
		$css = new NGRenderCSS ();
		$css->cacheId = 'navdropdown'.$id;
				
		if (!$css->fetchFromCache ()) {
			include_once NGConfig::pluginsPath () . '/ngpluginnavigation/ngpluginnavigationsuperdropdown/ngpluginnavigationsuperdropdown.php';
			...
			$css->templateFilename = 'ngpluginnavigation/ngpluginnavigationsuperdropdown/tpl/css.tpl';
			$css->template->assign ( 'settings', $settings );
			$css->template->assign ( 'config', NGPluginNavigationSuperDropDown::parseConfig($settings->$property));
