...
class NGPluginLayoutProExt {
	...
	public function render() {
		...
		$this->layout->javaScripts ['vilnius'] = NGUtil::prependRootPath ( 'classes/plugins/ngpluginlayout/styles/vilnius/js/vilnius.js' );
		...
		if (!$this->layout->topic->hideeyecatcher) {
            ...
            foreach ($bouquet->items as $item) {
                ...
                $pictures [] = NGLink::getPictureURL($item->displayPicture()->objectUID, $renderWidth, -1, NGPicture::Ratio16by9);
            }

            if (count($pictures) === 0 && $this->layout->settings->config ['options.galeryinheritance'] == 'true') {
                $this->layout->createImage('eyecatcher', $renderWidth, -1, NGPicture::Ratio16by9);
            }
